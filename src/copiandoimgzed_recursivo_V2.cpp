/* En este programa experimento distintas estrategias para guardar  imagenes en disco.
 ...
 Se va a utilizar la memoria de la GPU para guardar las imagenes.
...
int main(int argc, char **argv) {
	...
	std::string direccion ("/home/nvidia/discoaye");
	std::string nombre;
...
//			t2=(double)cv::getTickCount();
//			//zed.retrieveMeasure(depth_image_zed, sl::MEASURE_DEPTH);//VIEW_DEPTH; Con Measure_DEPTH devuelve una matriz de 32 bits, con view_depth, es de 8 bits. Para fines ilustrativos, usar el view
...
//			t2 = ((double)cv::getTickCount() - t2);
//			std::cout << "todo depth " << t2*1000/(cv::getTickFrequency())<< " ms." << std::endl;
			...
			if (t>t2){
				t2=t;
			std::cout << "tiempo maximo de computo " << t2*1000/(cv::getTickFrequency()) << " ms." << std::endl;
