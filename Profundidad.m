...
%c=a(S(1)*1/2:S(1),:)<30;
%aa=b&c;
% save('Precarga.mat','a','I','I2','outliers','IMAGENES','S')
% 
% clear all;clc;close all;load('Precarga.mat')
%IMAGENES=1; % QUITAR ESTO EN UN FUTURO
j=1;k=zeros(20-3,6);
S=size(I);Va=zeros(3,6);
for p=1:40 % hace un barrido en alturas con un gap de 200, calcula la variancia de las 3 ultimas
    umbral=p*50;
    umbral2=p*50+200;
    [Fx,Fy,k1,b1,k2,b2]=BusquedaRectas(a,umbral,umbral2,outliers,IMAGENES);   
    Va(j,:)=[Fx,Fy,k1,b1,k2,b2];
    if j==3
        j=1;
    end
    if p>2
%         disp('Pausado')
        ...
        % Mostrar rectas
%         figure(1)
%         Puntos=[Fx Fy;S(1) round(b1);Fx Fy; S(1) round(b2)]; hold on;
%         plot(Puntos(1:2,2),Puntos(1:2,1),'r','LineWidth',2); plot(Puntos(3:4,2),Puntos(3:4,1),'b','LineWidth',2);hold off
%         title('Superposicion, recas Pf e imagen')
%         texto=['\leftarrow i=',num2str(i)];
%         text(round(b1), S(1)-10*i, texto)
%         pause()
%         ka=k;
    end
%     pause()
end
m=isnan(k); n=isinf(1./k); mm=m|n; k(mm)=NaN; % Paso todos los ceros a NaN.
m=min(k); k2=k./m; %Normalizado
Su=sum(k2,2);d=min(Su); Imin=find(Su==min(Su)); % Sumo las variancias normalizadas y me quedo con la menor de todas 
indice=Imin(1)+1;
umbral=indice*50;
umbral2=indice*50+200;
IMAGENES=1;
[Fx,Fy,k1,b1,k2,b2]=BusquedaRectas(a,umbral,umbral2,outliers,IMAGENES);
IMAGENES=0;
